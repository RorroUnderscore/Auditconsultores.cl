...
<?php elseif($questionnaireMode==='use_template'): ?>
        ...
        <?php if(($qBuilder['name'] ?? '') !== ''): ?>
          ...
          <?php foreach($questions as $idx=>$q): ?><div style='border:1px solid #e6e9f0;border-radius:12px;padding:10px;margin-bottom:8px'><form method='post' style='display:grid;grid-template-columns:1fr 220px auto;gap:8px'><input type='hidden' name='action' value='q_update_question'><input type='hidden' name='institution_id' value='<?= (int)$selectedInstitutionId ?>'><input type='hidden' name='tab' value='cuestionarios'><input type='hidden' name='qmode' value='use_template'><input type='hidden' name='estate' value='<?= htmlspecialchars($activeQEstate ?? $estateFilter) ?>'><input type='hidden' name='idx' value='<?= (int)$idx ?>'><input data-q-qtext='1' name='question_text' value='<?= htmlspecialchars(qText($q)) ?>'><input data-q-qcat='1' name='question_category' placeholder='Categoría' value='<?= htmlspecialchars(qCategory($q)) ?>'><button class='btn gray' type='submit'>Guardar</button></form></div><?php endforeach; ?>
...
(function(){
  ...
  let t1=null;document.querySelectorAll('[data-qtpl-name]').forEach(el=>el.addEventListener('input',()=>{clearTimeout(t1);t1=setTimeout(()=>postDraft('qtpl_draft_sync',collectTpl()),200);}));
  let t2=null;
  const bindSyncBeforeSubmit=(form,cb)=>{if(!form)return;form.addEventListener('submit',async(e)=>{if(form.dataset.syncing==='1')return;e.preventDefault();form.dataset.syncing='1';try{await cb();}finally{form.submit();}});};
  document.querySelectorAll("form input[name='qmode'][value='create_template']").forEach(h=>{const f=h.closest('form');bindSyncBeforeSubmit(f,async()=>{clearTimeout(t1);await postDraft('qtpl_draft_sync',collectTpl());});});
  document.querySelectorAll("form input[name='qmode'][value='use_template'], form input[name='qmode'][value='scratch']").forEach(h=>{const f=h.closest('form');if(!f)return;const a=((f.querySelector(\"input[name='action']\")||{}).value||'').trim();if(['q_delete_estate','q_delete_question','q_move_question','q_inherit_questions'].includes(a))return;bindSyncBeforeSubmit(f,async()=>{clearTimeout(t2);await postDraft('q_draft_sync',collectQ());});});
  document.querySelectorAll("a.chip[href*='qmode=create_template&tpl_estate=']").forEach(a=>a.addEventListener('click',async(e)=>{e.preventDefault();clearTimeout(t1);await postDraft('qtpl_draft_sync',collectTpl());window.location.href=a.href;}));
  document.querySelectorAll("a.chip[href*='qmode=use_template&q_estate='], a.chip[href*='qmode=scratch&q_estate=']").forEach(a=>a.addEventListener('click',async(e)=>{e.preventDefault();clearTimeout(t2);await postDraft('q_draft_sync',collectQ());window.location.href=a.href;}));
})();
</script>
