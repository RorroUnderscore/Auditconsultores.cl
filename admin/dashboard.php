...
<?php elseif($questionnaireMode==='use_template'): ?>
        ...
        <?php if(($qBuilder['name'] ?? '') !== ''): ?>
          ...
          <?php foreach($questions as $idx=>$q): ?><div style='border:1px solid #e6e9f0;border-radius:12px;padding:10px;margin-bottom:8px'><form method='post' style='display:grid;grid-template-columns:1fr 220px;gap:8px'><input type='hidden' name='action' value='q_update_question'><input type='hidden' name='institution_id' value='<?= (int)$selectedInstitutionId ?>'><input type='hidden' name='tab' value='cuestionarios'><input type='hidden' name='qmode' value='use_template'><input type='hidden' name='estate' value='<?= htmlspecialchars($estateFilter) ?>'><input type='hidden' name='idx' value='<?= (int)$idx ?>'><input data-qtpl-qtext='1' name='question_text' value='<?= htmlspecialchars(qText($q)) ?>'><input name='question_category' placeholder='Categoría' value='<?= htmlspecialchars(qCategory($q)) ?>'></form></div><?php endforeach; ?>
