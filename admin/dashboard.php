...
(function(){
  ...
  const syncQuestionForm=(form)=>{const fd=new FormData(form);fetch('/admin/dashboard.php',{method:'POST',body:fd,credentials:'same-origin'}).catch(()=>{});};
  document.querySelectorAll("form input[name='action'][value='q_update_question']").forEach(a=>{const f=a.closest('form');if(!f)return;let tf=null;f.querySelectorAll(\"input[name='question_text'],input[name='question_category']\").forEach(i=>i.addEventListener('input',()=>{clearTimeout(tf);tf=setTimeout(()=>syncQuestionForm(f),180);}));});
  document.querySelectorAll("form input[name='action'][value='qtpl_update_question']").forEach(a=>{const f=a.closest('form');if(!f)return;let tf=null;f.querySelectorAll(\"input[name='question_text'],input[name='question_category']\").forEach(i=>i.addEventListener('input',()=>{clearTimeout(tf);tf=setTimeout(()=>syncQuestionForm(f),180);}));});
