        ...
        <?php if(count($templateEstates)>0): ?>
          ...
          <?php foreach($questions as $idx=>$q): ?>
            <div style='border:1px solid #e6e9f0;border-radius:12px;padding:10px;margin-bottom:8px'>
              <form method='post' style='display:grid;grid-template-columns:1fr 280px;gap:8px' onchange='this.submit()'>
...
<?php elseif($questionnaireMode==='use_template'): ?>
        ...
        <?php if(($qBuilder['name'] ?? '') !== ''): ?>
          ...
          <?php foreach($questions as $idx=>$q): ?><div style='border:1px solid #e6e9f0;border-radius:12px;padding:10px;margin-bottom:8px'><form method='post' style='display:grid;grid-template-columns:1fr 220px;gap:8px' onchange='this.submit()'><input type='hidden' name='action' value='q_update_question'><input type='hidden' name='institution_id' value='<?= (int)$selectedInstitutionId ?>'><input type='hidden' name='tab' value='cuestionarios'><input type='hidden' name='qmode' value='use_template'><input type='hidden' name='estate' value='<?= htmlspecialchars($estateFilter) ?>'><input type='hidden' name='idx' value='<?= (int)$idx ?>'><input name='question_text' value='<?= htmlspecialchars(qText($q)) ?>'><input name='question_category' placeholder='Categoría' value='<?= htmlspecialchars(qCategory($q)) ?>'></form></div><?php endforeach; ?>
          <form method='post'><input type='hidden' name='action' value='q_add_question'><input type='hidden' name='institution_id' value='<?= (int)$selectedInstitutionId ?>'><input type='hidden' name='tab' value='cuestionarios'><input type='hidden' name='qmode' value='use_template'><input type='hidden' name='estate' value='<?= htmlspecialchars($estateFilter) ?>'><label>Agregar pregunta</label><div style='display:grid;grid-template-columns:1fr 220px;gap:8px' onchange='this.submit()'><input name='question_text' required><input name='question_category' placeholder='Categoría'><button class='btn'>Guardar</button></div></form>
