...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
  ...
  try {
    if ($action === 'create_institution') {
      ...
      $_SESSION['q_builder'] = ['name' => '', 'source_template_id' => null, 'status' => 'draft', 'questions' => ['Directivos'=>[],'Docentes'=>[],'Apoderados'=>[],'Paradocentes'=>[]]];
      ...
      $_SESSION['q_builder'] = ['name' => (string)$tpl['name'], 'source_template_id' => $templateId, 'status' => 'draft', 'questions' => $qs];
      ...
      $pdo->prepare('INSERT INTO questionnaires(institution_id, project_id, name, source_template_id, status, created_at, updated_at) VALUES (?,?,?,?,?,?,?)')->execute([$institutionId, $projectId, $name, $builder['source_template_id'], $status, date('c'), date('c')]);
      ...
      $pdo->commit(); $_SESSION['q_builder'] = ['name' => '', 'source_template_id' => null, 'status' => 'draft', 'questions' => ['Directivos'=>[],'Docentes'=>[],'Apoderados'=>[],'Paradocentes'=>[]]];
    }
  } catch (Throwable $e) {
...
if (!isset($_SESSION['q_builder'])) $_SESSION['q_builder'] = ['name' => '', 'source_template_id' => null, 'status' => 'draft', 'questions' => ['Directivos'=>[],'Docentes'=>[],'Apoderados'=>[],'Paradocentes'=>[]]];
  ...
  <?php elseif($tab==='cuestionarios'): ?>
    <section class='card' style='margin-top:16px'><h3>Cuestionarios</h3><div class='card-body'>
      ...
      <?php else: ?>
        ...
        <div style='margin-top:10px;display:flex;gap:8px'><form method='post'><input type='hidden' name='action' value='q_save'><input type='hidden' name='institution_id' value='<?= (int)$selectedInstitutionId ?>'><input type='hidden' name='tab' value='cuestionarios'><input type='hidden' name='qmode' value='scratch'><button class='btn'>Guardar cuestionario</button></form><form method='post'><input type='hidden' name='action' value='q_publish'><input type='hidden' name='institution_id' value='<?= (int)$selectedInstitutionId ?>'><input type='hidden' name='tab' value='cuestionarios'><input type='hidden' name='qmode' value='scratch'><button class='btn gray'>Publicar cuestionario</button></form></div>
      <?php endif; ?>
    </div></section>
