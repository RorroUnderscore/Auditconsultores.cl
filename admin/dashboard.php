          ...
          <?php foreach($questions as $idx=>$q): ?>
            <div style='border:1px solid #e6e9f0;border-radius:12px;padding:10px;margin-bottom:8px'>
              <form method='post' style='display:grid;grid-template-columns:1fr 280px;gap:8px'>
                ...
                <input data-q-qtext='1' name='question_text' value='<?= htmlspecialchars(qText($q)) ?>'>
                <input data-qtpl-qcat='1' name='question_category' list='qtpl_categories' placeholder='Categoría (existente o nueva)' value='<?= htmlspecialchars(qCategory($q)) ?>'>
...
<?php elseif($questionnaireMode==='use_template'): ?>
        ...
        <?php if(($qBuilder['name'] ?? '') !== ''): ?>
          ...
          <?php foreach($questions as $idx=>$q): ?><div style='border:1px solid #e6e9f0;border-radius:12px;padding:10px;margin-bottom:8px'><form method='post' style='display:grid;grid-template-columns:1fr 220px;gap:8px'><input type='hidden' name='action' value='q_update_question'><input type='hidden' name='institution_id' value='<?= (int)$selectedInstitutionId ?>'><input type='hidden' name='tab' value='cuestionarios'><input type='hidden' name='qmode' value='use_template'><input type='hidden' name='estate' value='<?= htmlspecialchars($activeQEstate ?? $estateFilter) ?>'><input type='hidden' name='idx' value='<?= (int)$idx ?>'><input data-q-qtext='1' name='question_text' value='<?= htmlspecialchars(qText($q)) ?>'><input data-q-qcat='1' name='question_category' list='q_categories' placeholder='Categoría' value='<?= htmlspecialchars(qCategory($q)) ?>'></form></div><?php endforeach; ?>
          <form method='post'><input type='hidden' name='action' value='q_add_question'><input type='hidden' name='institution_id' value='<?= (int)$selectedInstitutionId ?>'><input type='hidden' name='tab' value='cuestionarios'><input type='hidden' name='qmode' value='use_template'><input type='hidden' name='estate' value='<?= htmlspecialchars($activeQEstate ?? $estateFilter) ?>'><label>Agregar pregunta</label><div style='display:grid;grid-template-columns:1fr 220px auto;gap:8px'><input name='question_text' required><input name='question_category' list='q_categories' placeholder='Categoría'><button class='btn'>Guardar</button></div></form>
          <div style='margin-top:10px;display:flex;gap:8px'><form method='post'><input type='hidden' name='action' value='q_save'><input type='hidden' name='institution_id' value='<?= (int)$selectedInstitutionId ?>'><input type='hidden' name='tab' value='cuestionarios'><input type='hidden' name='qmode' value='use_template'><button class='btn'>Guardar cuestionario</button></form><form method='post'><input type='hidden' name='action' value='q_publish'><input type='hidden' name='institution_id' value='<?= (int)$selectedInstitutionId ?>'><input type='hidden' name='tab' value='cuestionarios'><input type='hidden' name='qmode' value='use_template'><button class='btn gray'>Publicar cuestionario</button></form></div><datalist id='q_categories'><?php foreach($qCategories as $cat): ?><option value='<?= htmlspecialchars($cat) ?>'><?php endforeach; ?></datalist>
        <?php endif; ?>
      <?php elseif($questionnaireMode==='scratch' || $questionnaireMode==='institution_editor'): ?>
        ...
        </div><datalist id='q_categories'><?php foreach($qCategories as $cat): ?><option value='<?= htmlspecialchars($cat) ?>'><?php endforeach; ?></datalist>
      <?php else: ?>
...
(function(){
  ...
  let t2=null;document.querySelectorAll('[data-q-qtext],[data-q-qcat]').forEach(el=>el.addEventListener('input',()=>{clearTimeout(t2);t2=setTimeout(()=>{let estates=[...document.querySelectorAll("a.chip[href*='q_estate=']")].map(a=>a.textContent.trim()); if(estates.length===0){ estates=[...new Set([...document.querySelectorAll("form input[name='action'][value='q_update_question']")].map(a=>((a.closest('form').querySelector("input[name='estate']")||{}).value||'').trim()).filter(Boolean))]; }const questions={};document.querySelectorAll("form input[name='action'][value='q_update_question']").forEach(a=>{const f=a.closest('form');const estate=(f.querySelector("input[name='estate']")||{}).value||'';const qt=(f.querySelector("input[name='question_text']")||{}).value||'';const qc=(f.querySelector("input[name='question_category']")||{}).value||'';if(!questions[estate])questions[estate]=[];questions[estate].push({text:qt,category:qc});});postDraft('q_draft_sync',{estates,questions});},350);}));
})();
</script>
